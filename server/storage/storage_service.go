...
const (
	minChunkSizeBytes int64 = 1 << 20
	maxChunkSizeBytes int64 = 1 << 30
)
...
func (s *StorageServiceImpl) SetChunkSize(size int64) error {
	if size < minChunkSizeBytes || size > maxChunkSizeBytes {
		return fmt.Errorf("chunk_size_bytes must be between %d and %d", minChunkSizeBytes, maxChunkSizeBytes)
	}
