...
func (s *Server) proxyEndpoint(w http.ResponseWriter, r *http.Request, modelFinder func(body io.Reader) (model string, err error)) {
	...
	var allocations allocationViewer
	if viewer, ok := any(s.scheduler).(allocationViewer); ok {
		allocations = viewer
	}

	task := newTaskWithCompletion(newProxyTask(model, w, r, s.metrics, allocations))
